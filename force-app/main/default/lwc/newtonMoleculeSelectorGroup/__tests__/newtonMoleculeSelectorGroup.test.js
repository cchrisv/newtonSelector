...
import NewtonMoleculeSelectorGroup from "c/newtonMoleculeSelectorGroup";
import { MANUAL_INPUT_VALUE } from "c/newtonUtilitySelectorDataSources";
...
function mount(overrides = {}) {
  const el = createElement("c-newton-molecule-selector-group", {
    is: NewtonMoleculeSelectorGroup
  });
...
function dispatchCardSelect(hostEl, value) {
  const card = hostEl.shadowRoot.querySelector("c-newton-atom-choice-tile");
...
describe("c-newton-molecule-selector-group", () => {
  afterEach(() => {
  ...
  it("renders one tile per item", async () => {
    ...
    const cards = el.shadowRoot.querySelectorAll("c-newton-atom-choice-tile");
  ...
  it("renders picklist options as customizable cards", async () => {
    ...
    expect(trigger.querySelector("c-newton-atom-choice-tile")).toBeNull();
    ...
    const cards = el.shadowRoot.querySelectorAll(
      ".newton-picklist__menu c-newton-atom-choice-tile"
    );
    expect(cards).toHaveLength(3);
  ...
  it("renders radio as card-backed radio rows", async () => {
    ...
    const card = el.shadowRoot.querySelector("c-newton-atom-choice-tile");
  ...
  it("renders drag/drop columns with available and selected card panels", async () => {
    ...
    expect(
      available.querySelectorAll("c-newton-atom-choice-tile")
    ).toHaveLength(2);
    expect(selected.querySelector("c-newton-atom-choice-tile").item.value).toBe(
      "b"
    );
  });
  ...
  it("moves cards with controls in the Salesforce-style multi-select layout", async () => {
    ...
    dispatchCardSelectFrom(
      available.querySelector("c-newton-atom-choice-tile"),
      "a"
    );
    await Promise.resolve();
    ...
    dispatchCardSelectFrom(
      selected.querySelector("c-newton-atom-choice-tile"),
      "b"
    );
    await Promise.resolve();
  ...
  it("filters transfer layouts from the available panel", async () => {
    ...
    expect(
      available.querySelector("c-newton-atom-choice-tile").item.value
    ).toBe("c");
  });
  ...
  it("treats the none option as a clear action in every layout", async () => {
    ...
    dispatchCardSelectFrom(
      el.shadowRoot.querySelector("c-newton-atom-choice-tile"),
      ""
    );
