...
export default class NewtonAtomIcon extends LightningElement {
  @api name = "";
  @api size = "md";
  @api alternativeText = "";
  @api title = "";
  ...
  renderedCallback() {
    const svg = this.template.querySelector(".newton-icon__svg");
  ...
  get wrapperClass() {
    const cls = ["newton-icon", `newton-icon_size-${this.resolvedSize}`];
    if (VALID_BOXES.has(this.box)) {
      cls.push(`newton-icon_box-${this.box}`);
    }
    if (VALID_VARIANTS.has(this.variant)) {
      cls.push(`newton-icon_variant-${this.variant}`);
    }
    return cls.join(" ");
  }
