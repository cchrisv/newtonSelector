...
jest.mock(
  "c/newtonOrganismSelectorDataConfig",
  () => {
    const { LightningElement } = require("lwc");
    return class extends LightningElement {};
  },
...
jest.mock(
  "c/newtonOrganismSelectorContentConfig",
  () => {
    const { LightningElement } = require("lwc");
    return class extends LightningElement {};
  },
...
jest.mock(
  "c/newtonOrganismSelectorBehaviorConfig",
  () => {
    const { LightningElement } = require("lwc");
    return class extends LightningElement {};
  },
...
jest.mock(
  "c/newtonOrganismSelectorAppearanceConfig",
  () => {
    const { LightningElement } = require("lwc");
    return class extends LightningElement {};
  },
...
import NewtonFlowSelectorConfigModal from "c/newtonFlowSelectorConfigModal";
...
function mount(props = {}) {
  const element = createElement("c-newton-flow-selector-config-modal", {
    is: NewtonFlowSelectorConfigModal
  });
...
describe("c-newton-flow-selector-config-modal", () => {
  beforeEach(() => {
  ...
  it("initializes merged config and source refs for extracted chapters", () => {
    ...
    const data = child(element, "c-newton-organism-selector-data-config");
    const content = child(element, "c-newton-organism-selector-content-config");
    const behavior = child(
      element,
      "c-newton-organism-selector-behavior-config"
    );
  ...
  it("applies child configpatch and refchange events", async () => {
    ...
    const content = child(element, "c-newton-organism-selector-content-config");
    const nextData = child(element, "c-newton-organism-selector-data-config");
  ...
  it("tracks active chapter and splitter width from the studio template", async () => {
    const element = mount({ initialConfig: VALID_CONFIG });
    const studio = child(element, "c-newton-template-selector-studio");
    ...
    const nextStudio = child(element, "c-newton-template-selector-studio");
  ...
  it("tracks section clicks and toggles forced preview state", async () => {
    const element = mount({ initialConfig: VALID_CONFIG });
    const studio = child(element, "c-newton-template-selector-studio");
    const preview = child(element, "c-newton-organism-selector-config-preview");
    ...
    let nextStudio = child(element, "c-newton-template-selector-studio");
    let nextPreview = child(
      element,
      "c-newton-organism-selector-config-preview"
    );
    expect(
    ...
    nextPreview = child(element, "c-newton-organism-selector-config-preview");
