...
} from "c/newtonUtilityCpeHelpers";
import getObjectFields from "@salesforce/apex/NewtonSelectorCpeController.getObjectFields";
...
export default class NewtonOrganismResourceSelector extends LightningElement {
  @api name;
  @api label;
  @api required = false;
  ...
  processOptions(searchString) {
    ...
    if (this.allOptions && this.allOptions.length) {
      this.allOptions.forEach((curOption) => {
        ...
        // (e.g. `<c-newton-organism-resource-selector builder-context-filter-collection-boolean>`) arrive
