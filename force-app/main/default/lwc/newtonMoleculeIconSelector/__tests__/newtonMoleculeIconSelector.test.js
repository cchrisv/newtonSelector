...
import NewtonMoleculeIconSelector from "c/newtonMoleculeIconSelector";

function mount(overrides = {}) {
  const el = createElement("c-newton-molecule-icon-selector", {
    is: NewtonMoleculeIconSelector
  });
...
describe("c-newton-molecule-icon-selector", () => {
  afterEach(() => {
  ...
  it("does not render the combobox icon grid until opened", async () => {
    ...
    expect(el.shadowRoot.querySelectorAll(".newton-icon-cell")).toHaveLength(0);
    ...
    expect(el.shadowRoot.querySelectorAll(".newton-icon-cell").length).toBe(80);
  });
  ...
  it("fires iconselect with type:name format when an icon is clicked", async () => {
    ...
    await Promise.resolve();
    const firstIcon = el.shadowRoot.querySelector(".newton-icon-cell");
  ...
  it("filters icons by search term", async () => {
    const el = mount();
    await Promise.resolve();
    const before = el.shadowRoot.querySelectorAll(".newton-icon-cell").length;
    ...
    await Promise.resolve();
    const after = el.shadowRoot.querySelectorAll(".newton-icon-cell").length;
  ...
  it("pre-selects the matching entry when value is set", async () => {
    ...
    await Promise.resolve();
    const selected = el.shadowRoot.querySelector(".newton-icon-cell_selected");
  ...
  it("loads additional icon pages on request", async () => {
    ...
    expect(el.shadowRoot.querySelectorAll(".newton-icon-cell")).toHaveLength(
      80
    );
