 ...
 *
 * Field metadata is fetched through the shared LRU cache in `c/newtonUtilityCpeHelpers` so pickers on the same
...
export default class NewtonMoleculeFieldSelector extends LightningElement {
  ...
  _loadFields() {
    ...
    fetchFields(obj).then((fields) => {
      ...
      if (this.isMultiEntry) this._syncSelectedFromValue();
      const lu = this.template.querySelector("c-newton-molecule-custom-lookup");
