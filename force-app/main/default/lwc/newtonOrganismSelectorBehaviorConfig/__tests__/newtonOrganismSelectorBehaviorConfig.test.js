...
import NewtonOrganismSelectorBehaviorConfig from "c/newtonOrganismSelectorBehaviorConfig";
...
function mount(config = BASE_CONFIG) {
  const element = createElement("c-newton-organism-selector-behavior-config", {
    is: NewtonOrganismSelectorBehaviorConfig
  });
...
describe("c-newton-organism-selector-behavior-config events", () => {
  afterEach(() => {
  ...
  it("emits multi-select patch from the Mode toggle without changing compatible layout options", () => {
    ...
    toggle(
      byLabel(element.shadowRoot, "c-newton-atom-toggle", "Selection mode"),
      true
    );
  ...
  it("preserves the selected layout when selection mode changes", () => {
    ...
    toggle(
      byLabel(radio.shadowRoot, "c-newton-atom-toggle", "Selection mode"),
      true
    );
    ...
    toggle(
      byLabel(transfer.shadowRoot, "c-newton-atom-toggle", "Selection mode"),
      false
    );
  ...
  it("emits required, auto nav, search, and error message patches", () => {
    ...
    toggle(
      element.shadowRoot.querySelector(
        'c-newton-atom-toggle[data-key="required"]'
      ),
      true
    );
    toggle(
      element.shadowRoot.querySelector(
        'c-newton-atom-toggle[data-key="enableSearch"]'
      ),
      true
    );
    toggle(
      element.shadowRoot.querySelector(
        'c-newton-atom-toggle[data-key="autoAdvance"]'
      ),
      false
    );
    valueChanged(
      element.shadowRoot.querySelector("c-newton-organism-resource-selector"),
      "Pick one"
    );
  ...
  it("renders Mode, Required, Auto Nav, and Search as separate focused sections", () => {
    ...
    expect(
      byLabel(modeCard, "c-newton-atom-toggle", "Selection mode")
    ).not.toBeUndefined();
    expect(requiredCard).not.toBeUndefined();
    expect(
      requiredCard.querySelector(".newton-card__glyph c-newton-atom-icon").name
    ).toBe("asterisk");
    expect(
  ...
  it("does not render empty card bodies for off-state toggle sections", () => {
    ...
    expect(
      byLabel(element.shadowRoot, "c-newton-atom-toggle", "Required").closest(
        ".newton-studio__card"
      )
    ).not.toBeNull();
  });
  ...
  it("emits manual input patches", () => {
    ...
    toggle(
      byLabel(element.shadowRoot, "c-newton-atom-toggle", "Allow manual input"),
      false
    );
    inputChange(
